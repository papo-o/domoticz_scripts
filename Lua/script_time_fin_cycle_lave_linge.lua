...
local energy_consumption      = 'Conso Lave Linge'         --Name of Z-Wave plug that contains actual consumption of washingmachine (in Watts)
local washer_counter_uservar  = 'Compteur Lave Linge'        --Name of the uservariable that will contain the counter that is needed
local idle_minutes            = 4                               --The amount of minutes the consumption has to stay below the 'consumption_lower' value
local consumption_upper       = 15                              --If usage is higher than this value (Watts), the washingmachine has started
local consumption_lower       = 2                             --If usage is lower than this value (Watts), the washingmachine is idle for a moment/done washing
local name 					  = "Lave Linge"
...
--voir_les_logs("script_time_fin_cycle.lua",debugging)
...
if ((uservariables[washer_status_uservar] == 1) and uservariables[washer_counter_uservar] == 0) then
  ...
  commandArray['Mid Value']='Set Level 1'
